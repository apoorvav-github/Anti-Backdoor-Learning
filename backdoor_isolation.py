...
def test(opt, test_clean_loader, test_bad_loader, model_ascent, criterion, epoch):
    ...
    if epoch < opt.tuning_epochs + 1:
        log_root = opt.log_root + '/ABL' + opt.target_type +'_results_tuning_epochs.csv'
        test_process.append(
...
def save_checkpoint(state, epoch, is_best, opt):
    if is_best:
        filepath = os.path.join(opt.isolate_model_root, opt.model_name + r'_{}_tuning_epochs{}.tar'.format(opt.target_type, epoch))

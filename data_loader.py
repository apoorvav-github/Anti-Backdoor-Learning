...
class DatasetBD(Dataset):
    ...
    def addTrigger(self, dataset, target_label, inject_portion, mode, distance, trig_w, trig_h, trigger_type, target_type):
        ...
        for i in tqdm(range(len(dataset))):
            ...
            elif target_type == 'BadNet':
                img = np.array(data[0])
                height, width, channels = img.shape
                if i in perm: # if image in perm list, add trigger into img and change the label to trigger_label
                    for c in range(channels):
                        img[height-3, width-3, c] = 255
                        img[height-2, width-3, c] = 255
                        img[height-3, width-2, c] = 255
                        img[height-2, width-2, c] = 255
                        dataset_.append((img, target_label))
                else:
                    dataset_.append((img, data[1]))
                    
            elif target_type == "SIG":
                img = np.array(data[0])

                if i in perm and data[1] == target_label:
                    alpha = 0.4
                    delta = 30
                    f = 6

...
def test(opt, test_clean_loader, test_bad_loader, model_ascent, criterion, epoch):
    ...
    log_root = opt.log_root + '/ABL_'+ opt.target_type + '_unlearning.csv'
    test_process.append(
...
def train(opt):
    ...
    print('----------- Data Initialization of Clean Label--------------')
    data_path_isolation = os.path.join(opt.isolate_data_root, "{}_isolation{}%_{}_Examples.npy".format(opt.model_name,
                                                                                                    opt.isolation_ratio * 100, opt.target_type))
    data_path_other = os.path.join(opt.isolate_data_root, "{}_other{}%_{}_Examples.npy".format(opt.model_name,
                                                                                            100 - opt.isolation_ratio * 100, opt.target_type))
...
def save_checkpoint(state, epoch, is_best, opt):
    if is_best:
        filepath = os.path.join(opt.unlearning_root, opt.model_name + r'-{}-Unlearning_epochs{}.tar'.format(opt.target_type, epoch))
